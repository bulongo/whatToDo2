...
export const TaskBar = ({task}) => {
  let {taskHeading,taskItem,dateCreated,inProgress,completeState,selectedState} = task
  ...
  const handleSelect = () => {
    ...
    if(taskSelected == false){
      setTaskSelected((prev) => {return !prev})
      selectedState = true
      counter += 1
      if(counter > 1){
        setMenuState(true)
  ...
  return(
    <div className={taskSelected ? styles.taskBarSelected : styles.taskBar} onClick={(e) => handleSelect(e)}>
      <h3>{taskHeading}</h3>
      <p>{taskItem}</p>
      <span className={taskSelected ? styles.selected : styles.unSelected}></span>
